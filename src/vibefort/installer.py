...
def _build_hook_block() -> str:
    ...
    lines.append('        local ucache="$HOME/.vibefort/cache/update_check.json"')
    lines.append('        local cache="$HOME/.vibefort/cache/banner_short.txt"')
    lines.append('        # Regenerate if config or update cache is newer than banner cache')
    lines.append('        if [ ! -f "$cache" ] || [ "$cfg" -nt "$cache" ] || [ -f "$ucache" -a "$ucache" -nt "$cache" ]; then')
