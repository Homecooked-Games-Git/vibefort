...
def _build_hook_block() -> str:
    ...
    lines.append('        case "$PROMPT_COMMAND" in *_vibefort_title*) ;; *) PROMPT_COMMAND="_vibefort_title;${PROMPT_COMMAND:+ $PROMPT_COMMAND}";; esac')
    lines.append('        _vibefort_env_check() {')
    lines.append('            _VIBEFORT_LAST_DIR="${_VIBEFORT_LAST_DIR:-}"')
    ...
    lines.append('            fi')
    lines.append('        }')
    lines.append('        case "$PROMPT_COMMAND" in *_vibefort_env_check*) ;; *) PROMPT_COMMAND="_vibefort_env_check;${PROMPT_COMMAND:+ $PROMPT_COMMAND}";; esac')
    lines.append("    fi")
    lines.append("fi")
    ...
    lines.append('            if echo "$pasted" | command vibefort check-paste 2>/dev/null; then')
    lines.append('                zle .bracketed-paste')
    lines.append('            else')
    lines.append('                zle -M "VibeFort: paste blocked (suspicious content detected)"')
    lines.append('            fi')
    lines.append('        else')
    lines.append('            zle .bracketed-paste')
    lines.append('        fi')
